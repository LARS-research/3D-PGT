...
from graphgps.transform.transforms import (pre_transform_in_memory,
                                           ...
                                           clip_graphs_to_size)
from graphgps.loader.molecule_datasets import MoleculeDataset
from graphgps.loader.splitters import scaffold_split
from extended_feature import get_bond_angle_index, get_bond_angle
...
def load_dataset_master(format, name, dataset_dir):
    ...
    elif format == 'Chem_dataset':
        dataset = MoleculeDataset(dataset_dir, dataset=name)
        if name == 'tox21':
            dataset.num_tasks = 12
        # index_split default : scaffold
        smiles_list = pd.read_csv(dataset_dir + '/processed/smiles.csv',
                                  header=None)[0].tolist()
        train_idx, valid_idx, test_idx = scaffold_split(
            dataset, smiles_list, null_value=0, frac_train=0.8,
            frac_valid=0.1, frac_test=0.1)
        print('split via scaffold')
        dataset.split_idxs = [train_idx,
                              valid_idx,
                              test_idx]
