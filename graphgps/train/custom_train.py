...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for iter, batch in enumerate(loader):
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            _true = true
            _pred = pred_score
        elif cfg.dataset.name == 'geom_pre-train':
            bl_pred, bl_true, ba_pred, ba_true, da_pred, da_true, graph_pred, graph_true = model(batch)
            bl_loss, bl_pred_score = compute_loss(bl_pred, bl_true)
            ba_loss, ba_pred_score = compute_loss(ba_pred, ba_true)
            da_loss, da_pred_score = compute_loss(da_pred, da_true)
            graph_loss, graph_pred_score = compute_loss(graph_pred, graph_true)
            loss = bl_loss + ba_loss + da_loss + graph_loss
            _true = bl_true.detach().to('cpu', non_blocking=True)
            _pred = bl_pred_score.detach().to('cpu', non_blocking=True)
        else:
            pred, true = model(batch)
